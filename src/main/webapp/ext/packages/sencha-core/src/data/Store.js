...
Ext.define('Ext.data.Store', {
     ...
     *
     * **Note:** If your store has been filtered, getData() will return a filtered collection.
     *
     ...
     *
     * Use {@link #loadData}, {@link #add}, or {@link #insert} if records need to be 
    ...
    onCollectionRemove: function(collection, info) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            // erasing (informing server through its own proxy) records, or 
            // records that have been filtered out
            if (removed && !isMove && !record.phantom && !record.erasing && !data.contains(record)) {
