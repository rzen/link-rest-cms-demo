...
Ext.define('Ext.util.Positionable', {
    ...
    calculateConstrainedPosition: function (constrainTo, proposedPosition, local, proposedSize) {
        ...
        vector = me.getConstrainVector(constrainTo, proposedConstrainPosition, proposedSize, local);
     ...
     */
    getConstrainVector: function (constrainTo, proposedPosition, proposedSize, local) {
        var me = this,
            // Always get the localXY coords (we don't want to account for any absolute xy coords from the constraining cmp)!
            thisRegion = me.getRegion(local),
            vector = [0, 0],
            shadowSize = (me.shadow && me.constrainShadow && !me.shadowDisabled) ? me.shadow.getShadowSize() : undefined,
            overflowed = false,
            constraintInsets = me.constraintInsets,
            thisRegionTop, thisRegionRight, thisRegionBottom, thisRegionLeft,
            constrainToTop, constrainToRight, constrainToBottom, constrainToLeft, constrainSize;
        ...
        if (!(constrainTo instanceof Ext.util.Region)) {
            ...
            if (!local) {
                ...
                // Since we are constraining we may zero-out top and left positions (unless constrained cmp is being maximized).
                // For drag operations, top and left will be zero.
                constrainTo = new Ext.util.Region(0, constrainSize.width, constrainSize.height, 0);
                constrainTo.top = constrainTo.left = 0;
            }
        }
        ...
        // Only define the vars now since they could have been translated above if given a proposedPosition.
        thisRegionTop = thisRegion.top;
        thisRegionRight = thisRegion.right;
        thisRegionBottom = thisRegion.bottom;
        thisRegionLeft = thisRegion.left;

        constrainToTop = constrainTo.top;
        constrainToRight = constrainTo.right;
        constrainToBottom = constrainTo.bottom;
        constrainToLeft = constrainTo.left;

        // Set the size of this region to the proposed size.
        if (proposedSize) {
            thisRegionRight = thisRegionLeft + proposedSize[0];
            thisRegionBottom = thisRegionTop + proposedSize[1];
        }
        ...
        if (thisRegionRight > constrainToRight) {
            overflowed = true;
            // Overflowed the right.
            vector[0] = constrainToRight - thisRegionRight;
        }

        if (thisRegionLeft + vector[0] < constrainToLeft) {
            overflowed = true;
            // Overflowed the left, we'll need to zero-out the offset.
            vector[0] = constrainToLeft - thisRegionLeft;
        }

        // Constrain the Y coordinate by however much this Element overflows.
        if (thisRegionBottom > constrainToBottom) {
            overflowed = true;
            // Overflowed the bottom.
            vector[1] = constrainToBottom - thisRegionBottom;
        }

        if (thisRegionTop + vector[1] < constrainToTop) {
            overflowed = true;
            // Overflowed the top, we'll need to zero-out the offset.
            vector[1] = constrainToTop - thisRegionTop;
        }
     ...
     */
    getRegion: function (local) {
        var box = this.getBox(null, local);
